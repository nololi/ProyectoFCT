...
<div class="container-fluid">
	...
	<div class="row">	
		<div class="col-xs-10 col-xs-offset-1 col-md-4 col-md-offset-2 margen5ciento">			
			<?php				
				$consulta = "SELECT * from activarusuarios where cadenacomprobacion='" .$_GET['cadena']."'";
				$conexion = crearConexion();
				$resultado = $conexion->query($consulta)->fetch_assoc();			
				if($resultado['cadenacomprobacion']==$_GET['cadena']){
					echo "<h1>Cuenta activada </h1>";
					echo "<p><a href='../../login.php' class='btn btn-default'>Entrar</a> </p>";
					$conexion->query("UPDATE `usuario` SET `activo`='SI' WHERE email='".$resultado['email']."'");
					$conexion->query("DELETE  from activarusuarios  WHERE email='".$resultado['email']."'");
					
				}else{ //si ya está activo 
					echo "<p class='text-warning'>Enlace no disponible</p>";
				}
